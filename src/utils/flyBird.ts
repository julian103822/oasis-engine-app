/*
 * @Author       : zhangshifang@xdf.cn
 * @Date         : 2022-08-12 13:55:18
 * @LastEditors  : zhangshifang@xdf.cn
 * @LastEditTime : 2022-08-15 10:43:06
 * @Description  : 描述信息
 */
/**
 * @title Flappy Bird
 * @category 2D
 */
import type { Texture2D } from 'oasis-engine'
import {
  AssetType,
  BoxColliderShape,
  Camera,
...
export function startGame() {
  ...
  camera.orthographicSize = engine.canvas.height / 128 / 2
  ...
  engine.resourceManager
    ...
    .then((spineEntity: any) => {
      ...
      spineEntity.transform.setPosition(-1.5, 2, 3)
      state.setAnimation(0, 'sneering', true)
      skeleton.setSkinByName('fullskin/0101')
      skeleton.setSlotsToSetupPose()
      state.apply(skeleton)
      spineAnimation.scale = 0.003
    })
    // 创建一个lottie动画
  engine.resourceManager.load<Entity>({
    ...
    lottieEntity.transform.setScale(0.3, 0.3, 0.3)
    lottieEntity.transform.setPosition(2, 2.5, 3)
    lottie.play()
  })
...
class ScriptPipe extends Script {
  ...
  private _pipeHorizontalV = 0.003
  ...
  onAwake() {
    ...
    engine.on(GameEvent.checkHit, (birdY: number) => {
      const len = this._nowPipeArr.length
      for (let i = 0; i < len; i++) {
        const pipePos = this._nowPipeArr[i].transform.position
  ...
  /**
     ...
     */
  private _destroyPipe(pipeIndex = -1) {
...
class ScriptScore extends Script {
  ...
  private _showScore(scoreNumStr: string) {
    ...
    let nowEntityLen = scoreEntitys.length
...
class ScriptGround extends Script {
  ...
  /** Horizontal movement speed. */
  private _groundHorizontalV = 0.0082
...
class ScriptBird extends Script {
  ...
  private _reciprocalSliceHeight = 1
